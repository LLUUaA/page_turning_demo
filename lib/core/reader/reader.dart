...
class MyReader extends CustomPainter {
  MyReader({@required this.offset}) {
  ...
  static void handlePanDown(DragDownDetails _, Size size) {
    ...
    if (dy > 2 * boxH && dx > boxW) {
      MyReader.initPosition = POSITION_STYLE.BOTTOM_RIGHT;
    } else if (dy < boxH && dx > boxW) {
      MyReader.initPosition = POSITION_STYLE.TOP_RIGHT;
    } else if (dy > boxH && dy < 2 * boxH && dx > boxW && dx < 2 * boxW) {
      MyReader.initPosition = POSITION_STYLE.MID;
    } else if (dy > boxH && dy < 2 * boxH && dx > 2 * boxW) {
      MyReader.initPosition = POSITION_STYLE.RIGHT;
    } else {
      MyReader.initPosition = POSITION_STYLE.LEFT;
    }
    print('click POSITION_STYLE =${MyReader.initPosition}');
  }
